...
def games(request):
    ...
    if _type in [0, 1]:  # 0 for hot 1 for new
    ...
    elif _type in [2, 20]:  # 2 and 20 for played
...
def set_or_remove_favorite(token, h5game, flag):
    ...
    h5gameplay.save()
    Html5GamesFavoriteLog.objects.create(token=token, h5game=h5game, favorite=flag)
